...
class PerfilCommand(commands.Cog):
    ...
    @commands.slash_command(
        name="perfil",
        description="💰 | Vendas | Exibe o perfil de compras de um membro",
    ...
    async def perfil(
        ...
        referral_code = ReferralManager.code_for(target.id, target.display_name)
        if not user_stats.get("total_purchases"):
            if target.id == inter.author.id:
                msg = (
                    f"{emoji.interrogation} Você ainda não fez nenhuma compra.\n"
                    f"{emoji.unlock} Seu código de indicação: `{referral_code}`"
                )
            else:
                msg = (
                    f"{emoji.interrogation} {target.mention} ainda não fez nenhuma compra.\n"
                    f"{emoji.unlock} Código de indicação: `{referral_code}`"
                )
        ...
        referral_data = (db.get_document("referrals") or {}).get("codes", {}).get(referral_code, {})
        referral_text = (
            f"{emoji.unlock} **Código:** `{referral_code}`\n"
            f"{emoji.relations} **Indicações aprovadas:** `{referral_data.get('approved_uses', 0)}`\n"
            f"{emoji.coin} **Comissões:** `R$ {float(referral_data.get('total_commission', 0)):.2f}`"
        )
        ...
        if mode == "components":
            ...
            components.append(
                disnake.ui.Container(
                    disnake.ui.TextDisplay(f"## {emoji.unlock} Programa de Indicação\n{referral_text}"),
                    **container_kwargs
                )
            )
            ...
            embed.add_field(
                name=f"{emoji.unlock} Programa de Indicação",
                value=referral_text,
                inline=False
            )
