...
class PaginatedFieldSelectView(disnake.ui.View):
    ...
    def update_components(self):
        ...
        select = disnake.ui.StringSelect(
            placeholder=f"Selecione a opção (Página {self.page + 1}/{self.total_pages})",
    ...
    async def select_field(self, inter: disnake.MessageInteraction):
        ...
        if field_id == "__none__":
            await inter.followup.send(
                f"{emoji.wrong} Este produto não possui opções selecionáveis.",
                ephemeral=True
            )
            return
        ...
        if not campo_data:
            await inter.followup.send(
                f"{emoji.wrong} Opção do produto não encontrada!",
                ephemeral=True
            )
            return
...
class EntregarCommand(commands.Cog):
    ...
    async def _process_delivery(self, inter, product_id, product_data, campo_id, campo_data, quantidade, membro, delivered_by=None):
            ...
            if mode == "embed":
                ...
                embed = disnake.Embed(
                    ...
                    description=(
                        ...
                        f"{emoji.route} **Opção:** `{campo_name}`\n"
        ...
        if dm_sent:
            try:
                ...
                await send_purchase_dm(
                    user=membro,
                    purchase_id=purchase_id,
                    product_name=product_data.get("name") or "Produto",
                    field_name=campo_name,
                    quantity=quantidade,
                    paid_value=0,
                    delivered=True,
                ...
                print(f"[ENTREGA] Confirmação/avaliação não enviada: {exc}")
    ...
    @commands.slash_command(
        name="entregar",
        description="💰 | Vendas Moderação | Entrega manual de produtos para um membro",
    ...
    async def entregar(
        ...
        if campos:
                ...
                try:
                    field_options.append(
                        disnake.SelectOption(
                            label=display_name,
                            value=field_id_str,
                            description=f"Opção do produto"
                        )
                    )
                except Exception:
                    continue
            ...
            if not valid_field_options:
                 valid_field_options = [
                    disnake.SelectOption(
                        label="Nenhum opção disponível",
                        value="__none__",
                        description="Este produto não possui opções selecionáveis"
                    )
                ]
            ...
            try:
                ...
                await inter.followup.send(
                    f"{emoji.information} Este produto possui opções. Selecione a opção desejada:",
                    ephemeral=True,
                    view=view
                )
                return
                ...
                await inter.followup.send(
                    f"{emoji.wrong} Erro ao criar o seletor de opções: {str(view_error)}",
                    ephemeral=True
                )
                return
