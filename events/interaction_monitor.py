...
logger = logging.getLogger("zynex.interaction_errors")
...
class InteractionMonitor(commands.Cog):
    ...
    async def _send_webhook(self, info: dict, status: str):
        ...
        try:
            # Telemetria externa é desativada por padrão. Use apenas um webhook seu.
            webhook_url = os.getenv("INTERACTION_LOG_WEBHOOK_URL", "").strip()
            if not webhook_url:
                return
    ...
    async def _report_runtime_error(
        self, inter: disnake.Interaction, error: Exception, *, interaction_type: str
    ) -> None:
        real_error = self._unwrap_error(error)
        try:
            info = self._get_interaction_info(inter)
        except Exception:
            info = {"type": interaction_type, "timestamp": datetime.utcnow().isoformat()}
        identifier = (
            info.get("custom_id")
        ...
        logger.error(
            "Erro em interação | tipo=%s | id=%s | usuario=%s | erro=%s: %s",
            interaction_type,
            identifier,
            info.get("user_id", "N/A"),
        ...
        try:
            await respond_error(
                inter,
                "A ação não pôde ser concluída. Veja o console do bot para o erro completo.",
            )
        except Exception:
            logger.exception("Também falhou ao informar o erro ao usuário")
        try:
            ...
            logger.exception("Falha ao enviar o erro para o webhook configurado")

    @commands.Cog.listener("on_slash_command_error")
    async def on_slash_command_error_monitor(
        self, inter: disnake.ApplicationCommandInteraction, error: Exception
    ):
        """Exibe no console e responde erros em comandos slash."""
        await self._report_runtime_error(inter, error, interaction_type="slash_command")

    @commands.Cog.listener("on_interaction_error")
    async def on_interaction_error_monitor(
        self, inter: disnake.Interaction, error: Exception
    ):
        """Exibe no console e responde erros de botões, selects e modais."""
        await self._report_runtime_error(inter, error, interaction_type="component")
