...
class OnGuildChannelUpdate(commands.Cog):
    ...
    async def on_guild_channel_update(self, before: disnake.abc.GuildChannel, after: disnake.abc.GuildChannel):
        ...
        if not changes:
            return
        executor = await buscar_executor_auditlog(after.guild, [disnake.AuditLogAction.channel_update], lambda e: getattr(e.target, 'id', None) == after.id)
        ...
        linhas = [f'{emoji.textc} **Canal:** {after.mention} (`{after.id}`)', f"""{emoji.member} **Executor:** {executor_str}{(f" (`{getattr(executor, 'id', None)}`)" if executor else '')}""", '', '**Alterações:**']
