...
class OnMemberUpdateTimeout(commands.Cog):
    ...
    async def on_member_update(self, before: disnake.Member, after: disnake.Member):
        ...
        if after_to and (not before_to or after_to != before_to):
            executor = await buscar_executor_auditlog(after.guild, [disnake.AuditLogAction.member_update, disnake.AuditLogAction.member_disconnect], lambda e: getattr(e.target, 'id', None) == after.id)
            ...
            linhas = [f'{emoji.member} **Membro:** {after.mention} (`{after.id}`)', f'{emoji.clock} **Castigo aplicado até:** <t:{int(after_to.timestamp())}:f> (<t:{int(after_to.timestamp())}:R>)', f"""{emoji.member} **Executor:** {executor_str}{(f" (`{getattr(executor, 'id', None)}`)" if executor else '')}"""]
            await enviar_log(after.guild, canal_id, 'Logs de Castigos - Aplicados', linhas)
        elif before_to and (not after_to):
            executor = await buscar_executor_auditlog(after.guild, [disnake.AuditLogAction.member_update], lambda e: getattr(e.target, 'id', None) == after.id)
            executor_str = executor.mention if executor and hasattr(executor, 'mention') else str(executor) if executor else 'Não identificado'
            linhas = [f'{emoji.member} **Membro:** {after.mention} (`{after.id}`)', f'{emoji.unlock} **Castigo removido**', f"""{emoji.member} **Executor:** {executor_str}{(f" (`{getattr(executor, 'id', None)}`)" if executor else '')}"""]
