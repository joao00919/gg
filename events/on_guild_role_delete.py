...
class OnGuildRoleDelete(commands.Cog):
    ...
    async def on_guild_role_delete(self, role: disnake.Role):
        ...
        if not canal_id:
            return
        executor = await buscar_executor_auditlog(role.guild, [disnake.AuditLogAction.role_delete], lambda e: getattr(e.target, 'id', None) == role.id)
        ...
        linhas = [f'{emoji.role} **Cargo excluído:** **{role.name}** (`{role.id}`)', f'{emoji.wand} **Cor:** `{role.colour}`', f'{emoji.pin} **Menção:** `{role.mentionable}` | **Separado:** `{role.hoist}`', f"""{emoji.member} **Executor:** {executor_str}{(f" (`{getattr(executor, 'id', None)}`)" if executor else '')}"""]
