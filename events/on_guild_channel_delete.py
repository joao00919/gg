...
class OnGuildChannelDelete(commands.Cog):
    ...
    async def on_guild_channel_delete(self, channel: disnake.abc.GuildChannel):
        ...
        executor = await buscar_executor_auditlog(channel.guild, [disnake.AuditLogAction.channel_delete], lambda e: getattr(e.target, 'id', None) == channel.id)
        ...
        linhas = [f'{emoji.textc} **Canal excluído:** **{channel.name}**', f'{emoji.textc} **Tipo:** {tipo}', f'{emoji.textc} **ID:** `{channel.id}`', f"""{emoji.member} **Executor:** {executor_str}{(f" (`{getattr(executor, 'id', None)}`)" if executor else '')}"""]
