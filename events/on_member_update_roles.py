...
class OnMemberUpdateRoles(commands.Cog):
    ...
    async def on_member_update(self, before: disnake.Member, after: disnake.Member):
        ...
        if not adicionados and (not removidos):
            return
        executor = await buscar_executor_auditlog(after.guild, [disnake.AuditLogAction.member_role_update, disnake.AuditLogAction.member_update], lambda e: getattr(e.target, 'id', None) == after.id)
        ...
        if adicionados and canal_add:
            linhas = [f'{emoji.member} **Membro:** {after.mention} (`{after.id}`)', f"""{emoji.member} **Executor:** {executor_str}{(f" (`{getattr(executor, 'id', None)}`)" if executor else '')}"""]
