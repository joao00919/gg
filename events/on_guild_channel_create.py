...
class OnGuildChannelCreate(commands.Cog):
    ...
    async def on_guild_channel_create(self, channel: disnake.abc.GuildChannel):
        ...
        executor = await buscar_executor_auditlog(channel.guild, [disnake.AuditLogAction.channel_create], lambda e: getattr(e.target, 'id', None) == channel.id)
        ...
        linhas = [f"{emoji.textc} **Canal criado:** {(channel.mention if hasattr(channel, 'mention') else f'#{channel.name}')}", f'{emoji.textc} **Tipo:** {tipo}', f'{emoji.textc} **ID:** `{channel.id}`', f"{emoji.dir} **Categoria:** {(channel.category.mention if getattr(channel, 'category', None) else 'Sem categoria')}", f"""{emoji.member} **Executor:** {executor_str}{(f" (`{getattr(executor, 'id', None)}`)" if executor else '')}"""]
