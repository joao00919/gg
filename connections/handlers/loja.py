    ...
    async def get_orders(bot, payload: dict) -> dict:
        ...
        try:
            ...
            for order in orders.values():
                ...
                # Filter by date range (ISO-8601 or Unix timestamp, always normalized to UTC).
                raw_order_date = order.get("createdAt") or order.get("created_at") or order.get("timestamp")
                ...
                try:
                    order_date = parse_date(raw_order_date)
                    start_date = parse_date(filters.get("startDate"))
                    end_date = parse_date(filters.get("endDate"))
                except (ValueError, TypeError, OverflowError):
                    continue
                if start_date and (not order_date or order_date < start_date):
                    continue
                if end_date and (not order_date or order_date > end_date):
                    continue
