...
def get_product_description(product: dict, *, fallback: bool = True) -> str:
    """Retorna a descrição pública real do produto, normalizando cadastros antigos.

    A descrição pode existir em ``info.description``, no documento raiz ou nas
    opções/campos de versões antigas. Espaços em branco não contam como texto.
    """
    if not isinstance(product, dict):
        return "Produto disponível para compra." if fallback else ""

    info = product.get("info") or {}
    candidates = [info.get("description"), product.get("description")]
    for field in (product.get("campos") or {}).values():
        if isinstance(field, dict):
            candidates.extend((field.get("description"), field.get("pre_description")))

    for value in candidates:
        text = str(value or "").strip()
        if text and text.lower() not in {"não configurado ainda...", "nao configurado ainda..."}:
            return text

    if not fallback:
        return ""
    name = str(product.get("name") or "Produto").strip() or "Produto"
    return f"{name} disponível para compra."


def ensure_product_description(product: dict) -> str:
    """Normaliza a descrição no formato atual e devolve o texto salvo."""
    description = get_product_description(product, fallback=True)
    ...
    for field in (product.get("campos") or {}).values():
        ...
        if not str(field.get("pre_description") or "").strip():
            field["pre_description"] = description
    return description
