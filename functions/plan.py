...
def get_plan() -> str:
    ...
    """
    env_plan = os.getenv("BOT_PLAN", "").strip().lower()
    if env_plan in {"pro", "basic", "cloud", "free"}:
        return env_plan
    ...
    try:
        if os.path.exists(CONFIG_PATH):
            with open(CONFIG_PATH, 'r', encoding='utf-8') as f:
                data = json.load(f)
                return data.get("plan", "basic").lower()
    except Exception as e:
        print(f"Erro ao ler configs/config_plan.json: {e}")
    
    return "basic"  # Padrão seguro: sem integrações cloud externas
...
def is_free() -> bool:
    """Verifica se o plano é Free (PIX Manual e Carteira Integrada)."""
...
def should_allow_payment_provider(provider_key: str) -> bool:
    ...
    if plan == "free":
        # O PIX Manual não depende de API externa e fica disponível em todos os planos.
        return provider_key in {"pix_manual", "sync_wallet"}
...
def should_load_cloud() -> bool:
    """A verificação local do ZYNEX Cloud está disponível em todos os planos."""
    return True
...
def should_enable_cloud_button() -> bool:
    """Mantém o botão ZYNEX Cloud disponível para a verificação local."""
    return True
...
def should_enable_panel_button(button_name: str) -> bool:
    ...
    if plan == "cloud":
        # No plano cloud, apenas SyncCloud, Configurações e Personalização
