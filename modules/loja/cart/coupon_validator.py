...
class CouponValidator:
    ...
    def validate_product_coupon(
        ...
        if starts_at and now_ts < int(starts_at):
            return False, "Cupom ainda não está disponível", 0, None

        allowed_products = coupon_data.get("allowed_products") or []
        if allowed_products and product_id not in {str(value) for value in allowed_products}:
            return False, "Cupom não permitido para este produto", 0, None

        used_by = [str(value) for value in coupon_data.get("used_by", [])]
        max_per_user = int(coupon_data.get("max_uses_per_user") or 0)
        if max_per_user > 0 and used_by.count(str(user_id)) >= max_per_user:
            return False, "Limite de uso por usuário atingido", 0, None
        ...
        # Calcular desconto no backend
        discount_type = str(coupon_data.get("discount_type") or "percentage").lower()
        discount_value = float(coupon_data.get("discount_value", coupon_data.get("percent", 0)) or 0)
        if discount_type in {"fixed", "fixo", "valor"}:
            discount = discount_value
        else:
            discount = purchase_value * (discount_value / 100)
        max_discount = coupon_data.get("max_discount")
        if max_discount is not None:
            discount = min(discount, max(0.0, float(max_discount)))
        discount = max(0.0, min(discount, purchase_value))
    ...
    def validate_mass_coupon(
        ...
        if starts_at and datetime.now().timestamp() < float(starts_at):
            return False, "Cupom ainda não está disponível", 0, None
        ...
        used_by = [str(value) for value in coupon.get("used_by", [])]
        max_per_user = int(coupon.get("max_uses_per_user", 1) or 0)
        if max_per_user > 0 and used_by.count(str(user_id)) >= max_per_user:
            return False, "Você atingiu o limite de uso deste cupom", 0, None
    ...
    def use_product_coupon(product_id: str, coupon_code: str, user_id: int):
        """Marca o cupom de produto como usado com proteção contra corrida local."""
        with CouponValidator._lock:
            products = db.get_document("loja_products") or {}
            product = products.get(product_id, {})
            cupons = product.get("cupons", {})
            coupon_code_upper = coupon_code.upper()
            for cid, cdata in cupons.items():
                if cdata.get("name", "").upper() == coupon_code_upper:
                    cupons[cid]["uses_count"] = int(cupons[cid].get("uses_count", 0)) + 1
                    cupons[cid].setdefault("used_by", []).append(str(user_id))
    ...
    def use_mass_coupon(coupon_code: str, user_id: int):
        """Marca o cupom em massa como usado com proteção contra corrida local."""
        with CouponValidator._lock:
            data = db.get_document("loja_mass_coupons") or {}
            code_upper = coupon_code.upper()
            if code_upper in data.get("coupons", {}):
                data["coupons"][code_upper]["uses"] = int(data["coupons"][code_upper].get("uses", 0)) + 1
                data["coupons"][code_upper].setdefault("used_by", []).append(str(user_id))
