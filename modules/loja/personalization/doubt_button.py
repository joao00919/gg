...
class DoubtButtonSystem:
    ...
    @staticmethod
    def panel_doubt_button(inter: disnake.Interaction, back_custom_id: str | None = None) -> dict:
        """Painel de configuração do botão de dúvidas com retorno ao painel de origem."""
        data = db.get_document("loja_doubt_button") or {}
        if back_custom_id:
            data["_back_custom_id"] = back_custom_id
            db.save_document("loja_doubt_button", data)
        else:
            back_custom_id = str(data.get("_back_custom_id") or "Loja_Personalizar")
        mode = (db.get_document("custom_mode") or {}).get("mode", "components")
        if mode == "components":
            return DoubtButtonSystem._panel_components(inter, back_custom_id=back_custom_id)
        return DoubtButtonSystem._panel_embed(inter, back_custom_id=back_custom_id)
    
    @staticmethod
    def _panel_components(inter: disnake.Interaction, back_custom_id: str = "Loja_Personalizar") -> dict:
        data = db.get_document("loja_doubt_button") or {}
    ...
    @staticmethod
    def _panel_embed(inter: disnake.Interaction, back_custom_id: str = "Loja_Personalizar"):
        data = db.get_document("loja_doubt_button") or {}
        
        embed = disnake.Embed(
            title=f"{emoji.z0} Botão de Dúvidas",
