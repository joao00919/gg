...
class ConfigurarCupom(commands.Cog):
    ...
    def _panel_components(inter: disnake.MessageInteraction, product_id: str, coupon_id: str) -> dict:
        ...
        header_text = safe_textdisplay(f'# {emoji.z0}\n-# Painel > Loja > Produto > Cupom > **{coupon_name}**')
        return {'components': [disnake.ui.Container(disnake.ui.TextDisplay(header_text), disnake.ui.Separator(), disnake.ui.TextDisplay('**Informações do cupom**'), disnake.ui.Section(disnake.ui.TextDisplay(safe_textdisplay(f"""-# Nome: `{coupon.get('name', '-')[:50]}` | Status: `{status_str}`\n-# Desconto: `{coupon.get('percent', 0)}%` | Usos: `{coupon.get('uses_count', 0)}`{(f"/`{coupon.get('max_uses')}`" if coupon.get('max_uses') is not None else '')}\n-# Mín. carrinho: `{(utils.format_price_brl(coupon.get('min_cart')) if coupon.get('min_cart') is not None else 'Nenhum')}` | Máx. carrinho: `{(utils.format_price_brl(coupon.get('max_cart')) if coupon.get('max_cart') is not None else 'Nenhum')}`""")), accessory=button_ativo), disnake.ui.Separator(), disnake.ui.TextDisplay(f"\n-# Criado em: {utils.format_timestamp(coupon.get('created_at'))}\n-# Última edição: {utils.format_timestamp(coupon.get('updated_at'))}\n-# Expira em: {(utils.format_timestamp(coupon['expires_at']) if coupon.get('expires_at') else 'Ilimitado')}\n                "), disnake.ui.Separator(), disnake.ui.ActionRow(disnake.ui.Button(label='Editar informações', emoji=emoji.edit, custom_id=f'Loja_EditarCupom:{product_id}:{coupon_id}'), disnake.ui.Button(label='Avançados', emoji=emoji.settings2, custom_id=f'Loja_AvancadoCupom:{product_id}:{coupon_id}'), disnake.ui.Button(label='Apagar', emoji=emoji.delete, custom_id=f'Loja_ApagarCupom:{product_id}:{coupon_id}')), **container_kwargs), disnake.ui.ActionRow(disnake.ui.Button(label='Voltar', style=disnake.ButtonStyle.grey, emoji=emoji.back, custom_id=f'Loja_CuponsProduto:{product_id}'))]}
    ...
    def _panel_embed(inter: disnake.MessageInteraction, product_id: str, coupon_id: str) -> dict:
        ...
        status_str = _coupon_status(coupon)
        embed_description = f"""-# Painel > Loja > Produto > Cupom > **{coupon.get('name', coupon_id)}**\n\n**Informações do cupom**\n-# Nome: `{coupon.get('name', '-')}`\n-# Status: `{status_str}`\n-# Desconto: `{coupon.get('percent', 0)}%`\n-# Criado em: {utils.format_timestamp(coupon.get('created_at'))}\n-# Última edição: {utils.format_timestamp(coupon.get('updated_at'))}\n-# Expira em: `{(utils.format_timestamp(coupon['expires_at']) if coupon.get('expires_at') else 'Ilimitado')}`\n-# Usos: `{coupon.get('uses_count', 0)}`{(f"/`{coupon.get('max_uses')}`" if coupon.get('max_uses') is not None else '')}\n-# Mín. carrinho: `{(utils.format_price_brl(coupon.get('min_cart')) if coupon.get('min_cart') is not None else 'Nenhum')}` | Máx. carrinho: `{(utils.format_price_brl(coupon.get('max_cart')) if coupon.get('max_cart') is not None else 'Nenhum')}`\n"""
        embed = disnake.Embed(description=embed_description, **embed_kwargs)
        components = [disnake.ui.ActionRow(disnake.ui.Button(label='Editar', emoji=emoji.edit, custom_id=f'Loja_EditarCupom:{product_id}:{coupon_id}'), disnake.ui.Button(label='Avançado', emoji=emoji.commands, custom_id=f'Loja_AvancadoCupom:{product_id}:{coupon_id}'), disnake.ui.Button(label='Alternar ativo', emoji=emoji.reload, custom_id=f'Loja_ToggleCupom:{product_id}:{coupon_id}')), disnake.ui.ActionRow(disnake.ui.Button(label='Apagar', emoji=emoji.delete, custom_id=f'Loja_ApagarCupom:{product_id}:{coupon_id}'), disnake.ui.Button(label='Voltar', style=disnake.ButtonStyle.grey, emoji=emoji.back, custom_id=f'Loja_CuponsProduto:{product_id}'))]
