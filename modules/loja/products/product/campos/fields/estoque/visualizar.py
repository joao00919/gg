...
def panel_components(inter: disnake.MessageInteraction, product_id: str, field_id: str) -> dict:
    ...
    stock_qtd = "Infinito" if is_infinite else str(StockManager.get_available_stock(product_id, field_id) or 0)
    stock_style = str(field.get("stock_style") or "traditional").lower()
    stock_style_text = "Fantasma" if stock_style == "ghost" else "Tradicional"
    product_name = safe_textdisplay(field.get("name") or product.get("name") or product_id, 80)

    components = [
        disnake.ui.Container(
            disnake.ui.TextDisplay(f"# {emoji.z0}\n-# Painel > Loja > Produtos > {product_name} > Estoque"),
            disnake.ui.Separator(),
            disnake.ui.TextDisplay("Gerencie os itens de estoque entregues após a compra deste campo."),
            disnake.ui.Separator(),
            disnake.ui.TextDisplay(
                f"-# Estilo de Estoque: `{stock_style_text}`\n"
                f"-# Quantidade de Itens no estoque: `{stock_qtd}`"
            ),
            disnake.ui.Separator(),
            disnake.ui.ActionRow(
                ...
                disnake.ui.Button(label="Fantasma", emoji=emoji.ghost, custom_id=f"Promisse_EstoqueFantasma:{product_id}:{field_id}"),
            ...
            disnake.ui.ActionRow(
                ...
                disnake.ui.Button(label="Infinito", emoji=emoji.infinity, custom_id=f"Loja_Estoque_AddInfinite:{product_id}:{field_id}"),
...
def panel_embed(inter: disnake.MessageInteraction, product_id: str, field_id: str) -> dict:
    ...
    if is_infinite:
        ...
        description_text = safe_textdisplay(
            f"-# Painel > Loja > {product_name} > **Opção do Produto > Estoque**\n\n"
...
def export_stock_file(product_id: str, field_id: str) -> str | None:
    ...
    content += f"Opção do Produto: {campo_name}\n"
