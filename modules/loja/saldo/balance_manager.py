...
class BalanceManager:
    ...
    def add_balance(
        ...
        # Registrar depósito
        deposit_record = {
            "id": deposit_id or str(int(datetime.now(timezone.utc).timestamp())),
            "amount": amount,
            "bonus": bonus,
    ...
    def adjust_balance(
        ...
        """Aplica um crédito ou débito administrativo, inclusive saldo negativo.

        Este método é usado por estornos de comissão. Diferente de ``use_balance``,
        ele não bloqueia o débito quando o afiliado já gastou o valor recebido.
        """
        doc = BalanceManager._get_users_doc()
        users = doc.get("users", {})
        user_data = users.get(str(user_id), {
        ...
        user_data["balance"] = round(float(user_data.get("balance", 0)) + normalized_amount, 2)
