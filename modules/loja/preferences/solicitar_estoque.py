...
class StockMessageModal(disnake.ui.Modal):
    """Define o texto simples do painel de solicitação de estoque."""

    def __init__(self, current: str = ""):
        super().__init__(
            title="Definir mensagem",
            custom_id="Loja_Stock_Message_Modal",
            components=[
                disnake.ui.TextInput(
                    label="Mensagem", custom_id="message", value=current,
                    style=disnake.TextInputStyle.paragraph, required=True, max_length=1800,
                    placeholder="Digite a mensagem exibida acima do painel...",
                )
            ],
        )
    ...
    async def callback(self, inter: disnake.ModalInteraction):
        prefs = db.get_document("loja_preferences") or {}
        cfg = prefs.setdefault("stock_requests", {}).setdefault("panel_message", {})
        cfg["message"] = str(inter.text_values.get("message") or "").strip()
        db.save_document("loja_preferences", prefs)
        await respond_panel(inter, StockRequestPreferences.panel(inter), prefer_edit=True)


class StockEmbedBodyModal(disnake.ui.Modal):
    """Define o corpo do embed no formato mostrado no vídeo."""

    def __init__(self, current: dict | None = None):
        current = current or {}
        super().__init__(
            title="Definir corpo do Embed",
            custom_id="Loja_Stock_Embed_Modal",
            components=[
                disnake.ui.TextInput(label="Título", custom_id="title", value=str(current.get("title") or "Solicitar Estoque"), required=True, max_length=256),
                disnake.ui.TextInput(label="Descrição", custom_id="description", value=str(current.get("description") or "Clique no botão abaixo para solicitar reposição de estoque."), style=disnake.TextInputStyle.paragraph, required=True, max_length=3500),
                disnake.ui.TextInput(label="Cor (Hex)", custom_id="color", value=str(current.get("color") or "#5865F2"), required=False, max_length=7),
            ],
        )
    ...
    async def callback(self, inter: disnake.ModalInteraction):
        prefs = db.get_document("loja_preferences") or {}
        cfg = prefs.setdefault("stock_requests", {}).setdefault("panel_message", {})
        cfg["message_style"] = "embed"
        cfg["embed"] = {
            "title": str(inter.text_values.get("title") or "").strip(),
            "description": str(inter.text_values.get("description") or "").strip(),
            "color": str(inter.text_values.get("color") or "").strip(),
        }
        if cfg.get("image_url"):
            cfg["embed"]["image_url"] = cfg["image_url"]
        cfg.setdefault("button", {"label": "Solicitar Estoque", "style": "green", "emoji": "📦"})
        db.save_document("loja_preferences", prefs)
        await respond_panel(inter, StockRequestPreferences.panel(inter), prefer_edit=True)


class StockImageModal(disnake.ui.Modal):
    def __init__(self, current: str = ""):
        super().__init__(
            title="Definir imagem",
            custom_id="Loja_Stock_Image_Modal",
            components=[
                disnake.ui.TextInput(label="URL da imagem/arquivo", custom_id="image_url", value=current, required=True, max_length=500, placeholder="https://...")
            ],
        )
    ...
    async def callback(self, inter: disnake.ModalInteraction):
        prefs = db.get_document("loja_preferences") or {}
        cfg = prefs.setdefault("stock_requests", {}).setdefault("panel_message", {})
        url = str(inter.text_values.get("image_url") or "").strip()
        if not url.startswith(("http://", "https://")):
            return await inter.response.send_message(f"{emoji.wrong} Informe uma URL válida.", ephemeral=True)
        cfg["image_url"] = url
        cfg.setdefault("embed", {})["image_url"] = url
        cfg.setdefault("button", {"label": "Solicitar Estoque", "style": "green", "emoji": "📦"})
        db.save_document("loja_preferences", prefs)
        await respond_panel(inter, StockRequestPreferences.panel(inter), prefer_edit=True)


def _stock_reference_panel(inter: disnake.Interaction) -> dict:
    prefs = db.get_document("loja_preferences") or {}
    cfg = prefs.get("stock_requests", {}).get("panel_message", {})
    message_ready = bool(str(cfg.get("message") or "").strip())
    embed_ready = bool((cfg.get("embed") or {}).get("title") or (cfg.get("embed") or {}).get("description"))
    image_ready = bool(cfg.get("image_url") or (cfg.get("embed") or {}).get("image_url"))
    ...
    if colors.get("primary"):
        try:
            ...
            pass
    status = (
        "Configure a mensagem de solicitação de estoque e publique no canal desejado.\n\n"
        "**Configuração atual**\n"
        f"• **Mensagem:** {'Definida' if message_ready else 'Não definida'}\n"
        f"• **Embed:** {'Definido' if embed_ready else 'Não definido'}\n"
        f"• **Imagens/Arquivo:** {'Definida' if image_ready else 'Não definida'}"
    )
    return {"components": [
        disnake.ui.Container(
            disnake.ui.TextDisplay(f"# {getattr(emoji, 'zenyx2', emoji.z0)}\n-# Painel > Loja > Preferências > **Solicitar Estoque**"),
            disnake.ui.Separator(),
            disnake.ui.TextDisplay(status),
            disnake.ui.Separator(),
            disnake.ui.ActionRow(
                disnake.ui.Button(label="Definir mensagem", emoji=emoji.edit, custom_id="Loja_Stock_DefineMessage"),
                disnake.ui.Button(label="Limpar", style=disnake.ButtonStyle.danger, emoji=emoji.delete, custom_id="Loja_Stock_ClearMessage", disabled=not message_ready),
            ),
            disnake.ui.ActionRow(
                disnake.ui.Button(label="Definir corpo do Embed", emoji=getattr(emoji, 'colors', emoji.edit), custom_id="Loja_Stock_DefineEmbed"),
                disnake.ui.Button(label="Limpar", style=disnake.ButtonStyle.danger, emoji=emoji.delete, custom_id="Loja_Stock_ClearEmbed", disabled=not embed_ready),
            ),
            disnake.ui.ActionRow(
                disnake.ui.Button(label="Definir imagem", emoji=getattr(emoji, 'image', emoji.edit), custom_id="Loja_Stock_DefineImage"),
                disnake.ui.Button(label="Limpar", style=disnake.ButtonStyle.danger, emoji=emoji.delete, custom_id="Loja_Stock_ClearImage", disabled=not image_ready),
            ),
            disnake.ui.ActionRow(
                disnake.ui.Button(label="Visualizar mensagem", emoji=emoji.search, custom_id="Loja_Stock_Preview", disabled=not (message_ready or embed_ready)),
                disnake.ui.Button(label="Postar mensagem", style=disnake.ButtonStyle.success, emoji=getattr(emoji, 'correct', emoji.arrow), custom_id="Loja_Stock_Post", disabled=not (message_ready or embed_ready)),
            ),
            **kwargs,
        ),
        disnake.ui.ActionRow(disnake.ui.Button(label="Voltar", emoji=emoji.back, custom_id="Loja_Preferencias")),
    ]}
...
class StockRequestPreferences(commands.Cog):
    ...
    def _panel_components(inter: disnake.MessageInteraction) -> dict:
        return _stock_reference_panel(inter)

    @staticmethod
    def _panel_embed(inter: disnake.MessageInteraction) -> dict:
        # A referência usa Components V2; mantém a mesma interface também quando o modo global é embed.
        return _stock_reference_panel(inter)
    ...
    async def on_button_click(self, inter: disnake.MessageInteraction):
        ...
        if cid == "Loja_Stock_DefineMessage":
            cfg = (db.get_document("loja_preferences") or {}).get("stock_requests", {}).get("panel_message", {})
            return await inter.response.send_modal(StockMessageModal(str(cfg.get("message") or "")))
        if cid == "Loja_Stock_DefineEmbed":
            cfg = (db.get_document("loja_preferences") or {}).get("stock_requests", {}).get("panel_message", {})
            return await inter.response.send_modal(StockEmbedBodyModal(cfg.get("embed") or {}))
        if cid == "Loja_Stock_DefineImage":
            cfg = (db.get_document("loja_preferences") or {}).get("stock_requests", {}).get("panel_message", {})
            return await inter.response.send_modal(StockImageModal(str(cfg.get("image_url") or "")))
        if cid in {"Loja_Stock_ClearMessage", "Loja_Stock_ClearEmbed", "Loja_Stock_ClearImage"}:
            prefs = db.get_document("loja_preferences") or {}
            cfg = prefs.setdefault("stock_requests", {}).setdefault("panel_message", {})
            if cid.endswith("Message"):
                cfg.pop("message", None)
            elif cid.endswith("Embed"):
                cfg.pop("embed", None)
            else:
                cfg.pop("image_url", None)
                if isinstance(cfg.get("embed"), dict):
                    cfg["embed"].pop("image_url", None)
            db.save_document("loja_preferences", prefs)
            return await respond_panel(inter, self.panel(inter), prefer_edit=True)
        if cid == "Loja_Stock_Preview":
            cfg = (db.get_document("loja_preferences") or {}).get("stock_requests", {}).get("panel_message", {})
            preview_cfg = dict(cfg)
            preview_cfg.setdefault("message_style", "embed")
            embed_cfg = preview_cfg.setdefault("embed", {})
            if cfg.get("message") and not embed_cfg.get("description"):
                embed_cfg["description"] = cfg["message"]
            preview_cfg.setdefault("button", {"label": "Solicitar Estoque", "style": "green", "emoji": "📦"})
            await inter.response.defer(ephemeral=True)
            return await inter.followup.send(**(await _build_preview_message(preview_cfg)), ephemeral=True)
        if cid == "Loja_Stock_Post":
            return await inter.response.send_modal(StockRequestSendChannelModal())
