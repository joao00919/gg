...
class WebhooksCog(commands.Cog):
    ...
    def PunishmentPanelEmbed(self, inter: disnake.MessageInteraction):
        ...
        avancado = config.get("webhooks_avancado", {})
        punicao_atual = avancado.get("punicao", "ban")
        ...
        embed = disnake.Embed(
            ...
            description="Escolha a punição para quem atingir o limite de webhooks.",
            color=primary_color
        )

        options = self._get_punishment_options(punicao_atual)
        components = [
            disnake.ui.ActionRow(disnake.ui.Select(options=options, custom_id="ProtWebhooksPunishmentSelect")),
            disnake.ui.ActionRow(disnake.ui.Button(label="Voltar", emoji=emoji.back, custom_id="ProtWebhooks_Back"))
        ]
    ...
    def ImmuneRolePanelEmbed(self, inter: disnake.MessageInteraction):
        ...
        components = [
            disnake.ui.ActionRow(disnake.ui.RoleSelect(custom_id="ProtWebhooksImmuneRoleSelect", max_values=25)),
            disnake.ui.ActionRow(
                disnake.ui.Button(label="Voltar", emoji=emoji.back, custom_id="ProtWebhooks_Back"),
                disnake.ui.Button(label="Remover", emoji=emoji.delete, style=disnake.ButtonStyle.red, custom_id="ProtWebhooksImmuneRoleClear", disabled=not cargos_imunes)
            )
        ]
        return embed, components
    ...
    def ImmuneRolePanelComponents(self, inter: disnake.MessageInteraction) -> list:
        ...
        return [
            disnake.ui.Container(
                disnake.ui.TextDisplay(f"# {emoji.z0}\n-# Painel > Proteção > Webhooks > **Cargos Imunes**"),
                disnake.ui.Separator(),
    ...
    def LogChannelPanelEmbed(self, inter: disnake.MessageInteraction):
        ...
        components = [
            disnake.ui.ActionRow(disnake.ui.ChannelSelect(custom_id="ProtWebhooksLogChannelSelect", channel_types=[disnake.ChannelType.text])),
            disnake.ui.ActionRow(
                disnake.ui.Button(label="Voltar", emoji=emoji.back, custom_id="ProtWebhooks_Back"),
                disnake.ui.Button(label="Remover", emoji=emoji.delete, style=disnake.ButtonStyle.red, custom_id="ProtWebhooksLogChannelClear", disabled=not canal_logs_id),
                disnake.ui.Button(label="Criar para mim", style=disnake.ButtonStyle.blurple, emoji=emoji.wand, custom_id="ProtWebhooksLogChannelCreate"),
            )
        ]
        return embed, components
        ...
        components = [
            disnake.ui.ActionRow(
                disnake.ui.Select(
                    ...
                    custom_id="ProtWebhooksConfigSelect"
                ),
            ),
            disnake.ui.ActionRow(
                disnake.ui.Select(
                    ...
                    custom_id="ProtWebhooksConfigSelectAvancado"
                ),
            ),
    ...
    async def webhooks_dropdown_listener(self, inter: disnake.MessageInteraction):
        custom_id = inter.component.custom_id
        if "ProtWebhooks" not in custom_id:
            return
        
        if custom_id == "ProtWebhooksPunishmentSelect":
            await interactions.handle_punishment_select(self, inter)
            return
        if custom_id == "ProtWebhooksImmuneRoleSelect":
            await interactions.handle_immune_role_select(self, inter)
            return
        if custom_id == "ProtWebhooksLogChannelSelect":
    ...
    async def webhooks_button_listener(self, inter: disnake.MessageInteraction):
        custom_id = inter.component.custom_id
        if "ProtWebhooks" not in custom_id:
            return

        action_map = {
            "ProtWebhooks_Back": self.display_panel,
            "ProtWebhooksImmuneRoleClear": interactions.handle_immune_role_clear,
            "ProtWebhooksLogChannelClear": interactions.handle_log_channel_clear,
            "ProtWebhooksLogChannelCreate": interactions.handle_log_channel_create,
        }
        handler = action_map.get(custom_id)
        if handler:
            if custom_id == "ProtWebhooks_Back":
                await handler(inter)
            else:
                await handler(self, inter)
