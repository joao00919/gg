...
PROTECAO_OPCOES = [
    ("protecao_servidor", "Proteção do Servidor", emoji.shield, "Bloqueie links e convites e configure punições e exceções."),
...
class ProtectionGeralCog(commands.Cog):
    ...
    async def protecao_geral_dropdown_listener(self, inter: disnake.MessageInteraction):
        ...
        protection_cog_name = {
            "protecao_servidor": "ServidorProtectionCog",
    ...
    async def back_listener(self, inter: disnake.MessageInteraction):
        if getattr(inter.component, "custom_id", "") != "Back_To_Protection_Geral":
            return
        return await self.display_protecao_geral_panel(inter)
