...
class SetPromptModal(disnake.ui.Modal):
    def __init__(self, channel_id: int):
        ...
        super().__init__(title="Configurar Prompt do ZYNEX AI Chat", components=components)
...
class GroqKeyModal(disnake.ui.Modal):
    def __init__(self):
        super().__init__(
            title="Configurar API Key (Groq)",
            custom_id="AIChat_GroqKeyModal",
            components=[
                disnake.ui.TextInput(
    ...
    async def callback(self, inter: disnake.ModalInteraction):
        key = inter.text_values.get("groq_key", "").strip()
        if len(key) < 20:
            return await inter.response.send_message(f"{emoji.wrong} Chave inválida.", ephemeral=True)
        credentials = db.get_document("ai_credentials") or {}
        credentials["groq_api_key"] = key
        db.save_document("ai_credentials", credentials)
        try:
            import functions.ai_api as ai_api
            ai_api.GROQ_API_KEYS = [key]
        except Exception:
            pass
        await inter.response.send_message(f"{emoji.correct} API Key da Groq configurada para esta instalação.", ephemeral=True)
...
class AIChatCog(commands.Cog):
    ...
    def Painel() -> list[disnake.ui.Container]:
        ...
        resumo = (
            ...
            f"{getattr(emoji, 'lock', '🔐')} **Chave Groq:** `{'Configurada' if (db.get_document('ai_credentials') or {}).get('groq_api_key') else 'Não configurada'}`"
        )
...
# {emoji.z0}
-# Painel > Automações > **ZYNEX AI Chat**
...
Permite que a IA interaja com os usuários em um canal específico.
            ...
            disnake.ui.ActionRow(
                ...
                disnake.ui.Button(label="Configurar API Key (Groq)", style=disnake.ButtonStyle.blurple, emoji=getattr(emoji, "lock", "🔐"), custom_id="AIChat_ApiKey"),
            ),
            **container_kwargs
        )
    ...
    def PainelAdicionarChat() -> list[disnake.ui.Container]:
        ...
        return [
            disnake.ui.Container(
                disnake.ui.TextDisplay(f"""
# {emoji.z0}
-# Painel > Automações > ZYNEX AI Chat > **Adicionar Chat**
                """),
                disnake.ui.Separator(spacing=disnake.SeparatorSpacing.small),
                disnake.ui.ActionRow(
    ...
    def PainelEditarChat(inter: disnake.Interaction) -> list[disnake.ui.Container]:
        ...
        return [
            disnake.ui.Container(
                disnake.ui.TextDisplay(f"""
# {emoji.z0}
-# Painel > Automações > ZYNEX AI Chat > **Editar Chat**
                """),
                disnake.ui.Separator(spacing=disnake.SeparatorSpacing.small),
                disnake.ui.ActionRow(
                    disnake.ui.Select(
    ...
    def PainelConfigurarChat(inter: disnake.Interaction, channel_id: int) -> list[disnake.ui.Container]:
        ...
        return [
            disnake.ui.Container(
                disnake.ui.TextDisplay(f"""
# {emoji.z0}
-# Painel > Automações > ZYNEX AI Chat > **Configurar**
                """),
                disnake.ui.Separator(spacing=disnake.SeparatorSpacing.small),
                disnake.ui.TextDisplay(resumo),
                disnake.ui.Separator(spacing=disnake.SeparatorSpacing.small),
    ...
    def PainelCargoImune() -> list[disnake.ui.Container]:
        ...
        return [
            disnake.ui.Container(
                disnake.ui.TextDisplay(f"""
# {emoji.z0}
-# Painel > Automações > ZYNEX AI Chat > **Cargo Imune**
                """),
                disnake.ui.Separator(spacing=disnake.SeparatorSpacing.small),
    ...
    def PainelMudarCanal(inter: disnake.Interaction, old_channel_id: int) -> list[disnake.ui.Container]:
        ...
        return [
            disnake.ui.Container(
                disnake.ui.TextDisplay(f"""
# {emoji.z0}
-# Painel > Automações > ZYNEX AI Chat > **Mudar Canal**
                """),
                disnake.ui.Separator(spacing=disnake.SeparatorSpacing.small),
                disnake.ui.ActionRow(
                    disnake.ui.ChannelSelect(
    ...
    async def aichat_button_listener(self, inter: disnake.MessageInteraction):
        ...
        if custom_id == "AIChat_ApiKey":
            await inter.response.send_modal(GroqKeyModal())
            return
