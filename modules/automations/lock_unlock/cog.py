...
class ConfigurarLockUnlockModal(disnake.ui.Modal):
    ...
    async def callback(self, inter: disnake.ModalInteraction):
        ...
        try:
            ...
            error_text = f"{emoji.wrong} Horário inválido. Use o formato `HH:MM`, entre `00:00` e `23:59`."
            try:
...
class LockUnlockCog(commands.Cog):
    ...
    async def on_button_click(self, inter: disnake.MessageInteraction):
        ...
        if cid == "LockUnlock_DesativarLogsViaLog":
            config_bot = db.obter("config.json")
            if str(inter.author.id) not in config_bot.get("bot", {}).get("perms", []):
