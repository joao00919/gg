...
def has_moderation_permission(user: disnake.User | disnake.Member) -> bool:
    """Verifica se o usuário tem permissão para moderar sugestões lendo o config.json e cargos.json."""
    try:
        with open('config.json', 'r', encoding='utf-8') as f:
            config = json.load(f)
        
        bot_config = config.get("bot", {})
        owner_id = bot_config.get("owner")
        perm_ids = bot_config.get("perms", [])
        user_id_str = str(user.id)

        if user_id_str == owner_id or user_id_str in perm_ids:
            return True
