...
class MensagensCargos:
    ...
    def cargo_criado_components(cargo: disnake.Role, auto: bool) -> disnake.ui.Container:
        return disnake.ui.Container(disnake.ui.TextDisplay(f'# {emoji.z0}\n-# Criar Todos os Cargos Automáticamente > Cargo Criado' if auto else f'# {emoji.z0}\n-# Criar Cargo > Cargo Criado'), disnake.ui.Separator(), disnake.ui.TextDisplay(f'**Informações do cargo:**\nID: `{cargo.id}`\nNome: `{cargo.name}`\nMenção: {cargo.mention}'))
    ...
    def cargo_criado_embed(cargo: disnake.Role, auto: bool):
        ...
        embed = disnake.Embed(title=f'Cargo Criado', description=f'**Informações do cargo:**\nID: `{cargo.id}`\nNome: `{cargo.name}`\nMenção: {cargo.mention}')
    ...
    def cargos_criados_embed(criados: list[disnake.Role]):
        colors = db.get_document('custom_colors')
        primary_color_hex = colors.get('primary')
        embed = disnake.Embed(title=f'Cargos Criados', description=f'`{len(criados)}` cargos criados com sucesso.')
        if primary_color_hex:
            primary_color = int(primary_color_hex.replace('#', ''), 16)
            embed.color = primary_color
        embed.add_field(name='Cargos:', value=f"{', '.join((f'{c.mention} (`{c.id}`)' for c in criados))}")
        return (embed, [])
        ...
        if inter.component.custom_id == 'Configuracoes_CriarTodosCargos':
            ...
            for key, label, _ in CARGOS_OPCOES:
                ...
                if existing_role_by_name:
                    ...
                    continue
                try:
                    cargo = await guild.create_role(name=label, reason=f'Auto-criação pelo painel de configurações - {inter.author.name} ({inter.author.id})', color=disnake.Color(CARGOS_CORES[random.randint(0, len(CARGOS_CORES) - 1)]))
