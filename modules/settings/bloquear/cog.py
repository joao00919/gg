...
class ConfigurarBlacklist(commands.Cog):
    ...
    @staticmethod
    def panel(inter: disnake.Interaction, back_custom_id: str | None = None) -> dict:
        blacklist_doc = db.get_document("blacklist") or {"ids": []}
        if back_custom_id:
            blacklist_doc["_back_custom_id"] = back_custom_id
            db.save_document("blacklist", blacklist_doc)
        else:
            back_custom_id = str(blacklist_doc.get("_back_custom_id") or "Painel_Configuracoes")
