...
class MensagensCanais:
    ...
    def canal_criado_components(ch: disnake.TextChannel, auto: bool) -> disnake.ui.Container:
        return disnake.ui.Container(disnake.ui.TextDisplay(f'# {emoji.z0}\n-# Criar Todos os Canais Automáticamente > Canal Criado' if auto else f'# {emoji.z0}\n-# Criar Canal > Canal Criado'), disnake.ui.Separator(), disnake.ui.TextDisplay(f'**Informações do canal:**\nID: `{ch.id}`\nNome: `{ch.name}`\nMenção: {ch.mention}'))

    @staticmethod
    def canais_criados_components(criados: list[disnake.TextChannel]) -> list[disnake.ui.Container]:
        return [disnake.ui.Container(disnake.ui.TextDisplay(f'\n**Informações dos canais criados:**\n`{len(criados)}` canais criados com sucesso.\n                '), disnake.ui.Separator(), disnake.ui.TextDisplay(f"{', '.join((f'{c.mention} (`{c.id}`)' for c in criados))}"))]

    @staticmethod
    def canal_criado_embed(ch: disnake.TextChannel, auto: bool):
        colors = db.get_document('custom_colors')
        primary_color_hex = colors.get('primary')
        embed = disnake.Embed(title=f'Canal Criado', description=f'**Informações do canal:**\nID: `{ch.id}`\nNome: `{ch.name}`\nMenção: {ch.mention}')
    ...
    def canais_criados_embed(criados: list[disnake.TextChannel]):
        colors = db.get_document('custom_colors')
        primary_color_hex = colors.get('primary')
        embed = disnake.Embed(title=f'Canais Criados', description=f'`{len(criados)}` canais criados com sucesso.')
        if primary_color_hex:
            primary_color = int(primary_color_hex.replace('#', ''), 16)
            embed.color = primary_color
        embed.add_field(name='Canais:', value=f"{', '.join((f'{c.mention} (`{c.id}`)' for c in criados))}")
        return (embed, [])
    ...
    async def criar_todos_canais(self, inter: disnake.MessageInteraction):
        if inter.component.custom_id == 'Configuracoes_CriarTodosCanais':
            ...
            try:
                ...
                cat_logs = None
            FORCE_LOGS = {'canal_de_evento_de_compras', 'canal_de_boas_vindas', 'canal_de_feedback', 'canal_de_logs_de_pedidos'}
            ...
            for key, label, _ in CANAIS_OPCOES:
                ...
                try:
                    ch = await guild.create_text_channel(name, overwrites=overwrites, category=category, reason=f'Auto-criação pelo painel de configurações - {inter.author.name} ({inter.author.id})')
                    defs[key] = str(ch.id)
                    criados.append(ch)
                    if mode == 'embed':
