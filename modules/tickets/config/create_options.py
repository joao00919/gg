...
class AddOptionModal(disnake.ui.Modal):
    ...
    async def callback(self, inter: disnake.ModalInteraction):
        ...
        option_name = (inter.text_values.get("option_name") or "").strip()
        option_description = (inter.text_values.get("option_description") or "").strip()
        if len(option_name) < 2:
            return await message.error(inter, "Informe um nome válido para a opção.")
        if not option_description:
            return await message.error(inter, "Informe uma descrição para a opção.")
        ...
        if any(str(item.get("name", "")).strip().casefold() == option_name.casefold() for item in panel["options"]):
            return await message.error(inter, "Já existe uma opção com esse nome neste painel.")
        ...
        new_option = {
            ...
            "open_message": {"style": "embed", "embed": {}, "content": {}, "container": {}},
        }
        if option_emoji:
