...
class EditIAPromptModal(disnake.ui.Modal):
    def __init__(self, panel_id: str):
        ...
        super().__init__(title="Editar Instruções da PromisseAI", components=components)
...
def ConfigIAView_components(inter: disnake.Interaction, panel_id: str) -> list:
    ...
    header_text = f"# {emoji.z0}\n-# Gerenciar Tickets > Editar Painel > {panel_name} > **PromisseAI**"
    status_text = (
...
def ConfigIAView_embed(inter: disnake.Interaction, panel_id: str):
    ...
    embed = disnake.Embed(
        title=f"Configurar PromisseAI: {panel_data.get('name')}",
