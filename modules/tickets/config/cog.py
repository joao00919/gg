...
from .ticket_mode import TicketModeView_components, TicketModeView_embed, save_ticket_mode
...
class TicketConfigCog(commands.Cog):
    ...
    async def display_ticket_panel(self, inter: disnake.MessageInteraction):
        ...
        if mode == "embed":
            ...
            panel = {"components": PainelTicket_components(inter)}
    ...
    async def ticket_button_listener(self, inter: disnake.MessageInteraction):
        ...
        if not has_capability(inter, "tickets"):
            return await inter.response.send_message(
                f"{emoji.wrong} Você não possui permissão para gerenciar tickets.",
                ephemeral=True,
            )
        ...
        if not inter.response.is_done():
            await respond_error(inter, "Ação de ticket não reconhecida ou indisponível.")
    ...
    async def handle_panel_edit_actions(self, inter, action, panel_id):
        ...
        if action == "TicketMode":
            await self._mode_aware_wait(inter)
            if mode == "components":
                await inter.edit_original_message(components=TicketModeView_components(inter, panel_id))
            else:
                embed, components = TicketModeView_embed(inter, panel_id)
                await self._handle_embed_edit(inter, embed, components)
        elif action == "ConfigIA":
            await self._mode_aware_wait(inter)
            if mode == "components":
            ...
            panels[panel_id]["has_pending_changes"] = True
            db.save_document("tickets_config", config)
            if mode == "components":
                await inter.edit_original_message(components=SpecificPanelView_components(inter, panel_id))
            else:
            ...
            if mode == "components":
                ...
                embed, components = SpecificPanelView_embed(inter, panel_id)
                await self._handle_embed_edit(inter, embed, components)
        elif action == "DeleteTickets":
            return await inter.response.send_message(
                content=(
                    f"{emoji.warn} **Deletar todos os tickets deste painel?**\n"
                    "Esta ação fecha e remove os canais/tópicos ainda registrados neste painel."
                ),
                components=[
                    disnake.ui.ActionRow(
                        disnake.ui.Button(
                            ...
                            emoji=emoji.delete,
                            custom_id=f"TicketEdit_ConfirmDeleteTickets_{panel_id}",
                        ),
                        disnake.ui.Button(
                            label="Cancelar",
                            style=disnake.ButtonStyle.grey,
                            emoji=emoji.back,
                            custom_id=f"TicketEdit_CancelDeleteTickets_{panel_id}",
                        ),
                    )
                ],
                ephemeral=True,
            )
        elif action == "CancelDeleteTickets":
            return await inter.response.edit_message(
                content=f"{emoji.correct} Exclusão cancelada.", components=[]
            )
        elif action == "ConfirmDeleteTickets":
            await inter.response.defer(ephemeral=True)
            tickets_data = db.get_document("tickets_data") or {}
            panel_tickets = (tickets_data.get("panels") or {}).get(panel_id, {}) or {}
            deleted = 0
            failed = 0
            for records in list(panel_tickets.values()):
                if not isinstance(records, list):
                    continue
                for ticket in records:
                    if not isinstance(ticket, dict):
                        continue
                    channel_id = ticket.get("ticket_id") or ticket.get("channel_id")
                    ...
                    try:
                        ...
                        await channel.delete(reason=f"Exclusão em massa do painel de tickets {panel_id}")
                        deleted += 1
                    except (disnake.NotFound, disnake.Forbidden):
                        failed += 1
                    except Exception:
                        failed += 1
            tickets_data.setdefault("panels", {}).pop(panel_id, None)
            db.save_document("tickets_data", tickets_data)
            return await inter.edit_original_message(
                content=(
                    f"{emoji.correct} Tickets removidos: **{deleted}**"
                    + (f" • Não encontrados/sem permissão: **{failed}**" if failed else "")
                ),
                components=[],
            )
        elif action == "Delete":
    ...
    async def ticket_dropdown_listener(self, inter: disnake.MessageInteraction):
        custom_id = inter.component.custom_id
        if not (custom_id.startswith("Ticket") or custom_id.startswith("ticket_panel_") or custom_id.startswith("select_panel_to_edit_")):
            return
        if (custom_id.startswith("Ticket") or custom_id.startswith("ticket_panel_")) and not has_capability(inter, "tickets"):
            return await inter.response.send_message(
                f"{emoji.wrong} Você não possui permissão para gerenciar tickets.",
                ephemeral=True,
            )
        
        if custom_id.startswith("TicketMode_Select_"):
            panel_id = custom_id.replace("TicketMode_Select_", "", 1)
            selected_mode = inter.values[0]
            await self._mode_aware_wait(inter)
            if not save_ticket_mode(panel_id, selected_mode):
                return await inter.edit_original_message(content=f"{emoji.wrong} Não foi possível salvar o tipo de ticket.", components=[])
            mode = db.get_document("custom_mode").get("mode")
            if mode == "components":
                await inter.edit_original_message(components=TicketModeView_components(inter, panel_id))
            else:
        ...
        elif custom_id.startswith("select_panel_to_edit_"):
            await self._mode_aware_wait(inter)
        ...
        if not inter.response.is_done():
            await respond_error(inter, "Seleção de ticket não reconhecida ou indisponível.")
