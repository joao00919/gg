...
def _find_ticket_record(channel_id: int):
    """Retorna (documento, ticket, owner_id) sem alterar o cliente do ticket."""
        ...
        for owner_id, tickets in (users or {}).items():
            for ticket in tickets or []:
                if ticket.get("ticket_id") == channel_id and ticket.get("status") == "open":
                    return tickets_data, ticket, int(owner_id)
    return tickets_data, None, None


class TransferAttendantSelect(disnake.ui.UserSelect):
    def __init__(self):
        super().__init__(
            placeholder="Selecione o novo atendente responsável",
            min_values=1,
            max_values=1,
            custom_id="ticket_transfer_attendant_select",
        )

    async def callback(self, inter: disnake.MessageInteraction):
        try:
            member_id = int(inter.values[0])
            member = inter.guild.get_member(member_id) or await inter.guild.fetch_member(member_id)
        except (ValueError, IndexError, disnake.NotFound):
            return await inter.response.send_message(
                f"{emoji.wrong} Atendente inválido ou não encontrado.",
                ephemeral=True,
            )

        if member.bot:
            return await inter.response.send_message(
                f"{emoji.wrong} Um bot não pode ser responsável pelo atendimento.",
                ephemeral=True,
            )
        if not await check_attendant_permissions(member, inter.channel.id):
            return await inter.response.send_message(
                f"{emoji.wrong} O membro selecionado não possui permissão de suporte.",
                ephemeral=True,
            )

        tickets_data, ticket, _owner_id = _find_ticket_record(inter.channel.id)
        if not ticket:
            return await inter.response.send_message(
        ...
        previous_id = ticket.get("assigned_to") or ticket.get("assumed_by")
        if previous_id and int(previous_id) == member.id:
            return await inter.response.send_message(
                f"{emoji.interrogation} {member.mention} já é responsável por este atendimento.",
                ephemeral=True,
            )

        ticket["assigned_to"] = member.id
        ticket["assumed_by"] = member.id
        ticket["last_staff_response_timestamp"] = int(disnake.utils.utcnow().timestamp())
        ticket["stale_alerted_at"] = None
        db.save_document("tickets_data", tickets_data)

        # Garante acesso em canais privados, mesmo quando o cargo foi alterado depois.
        try:
            if isinstance(inter.channel, disnake.Thread):
                if member not in inter.channel.members:
                    await inter.channel.add_user(member)
            else:
                await inter.channel.set_permissions(
                    member,
                    read_messages=True,
        ...
        log_ticket_event(
            channel_id=inter.channel.id,
            event_type="transfer_attendant",
            author_id=inter.author.id,
            details={
                "old_attendant_id": previous_id,
                "new_attendant_id": member.id,
            },
        )
        previous = inter.guild.get_member(int(previous_id)) if previous_id else None
        previous_text = previous.mention if previous else "`Não assumido`"
        await inter.channel.send(
            f"{emoji.back} **Atendimento transferido**\n"
            f"Responsável anterior: {previous_text}\n"
            f"Novo responsável: {member.mention}\n"
            f"Transferido por: {inter.author.mention}"
        )
        try:
            await member.send(
                f"{emoji.verified} Você recebeu o atendimento `{inter.channel.name}`.",
        ...
        await inter.response.edit_message(
            content=f"{emoji.correct} Atendimento transferido para {member.mention}.",
            view=None,
        )


class TransferAttendantView(disnake.ui.View):
    def __init__(self):
        super().__init__(timeout=180)
        self.add_item(TransferAttendantSelect())


async def transfer_attendant_ticket(inter: disnake.MessageInteraction):
    if not await check_attendant_permissions(inter.author, inter.channel.id):
        return await inter.response.send_message(
            f"{emoji.wrong} Você não tem permissão para transferir o atendimento.",
            ephemeral=True,
        )
    await inter.response.send_message(
        f"{emoji.verified} Selecione o novo atendente responsável.",
        view=TransferAttendantView(),
        ephemeral=True,
    )
