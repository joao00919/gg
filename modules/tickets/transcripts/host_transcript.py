"""
Sistema de hospedagem de transcripts usando a API de transcripts existente
...
def get_transcript_api_url() -> str:
    """Retorna a URL da API de transcripts, priorizando a variável de ambiente."""
    env_url = os.getenv("TRANSCRIPT_API_URL", "").strip()
    if env_url:
    ...
    try:
        ...
        api_host = str(config.get('transcripts', 'localhost:22222')).strip()
...
def get_transcript_api_headers() -> dict:
    """Monta o cabeçalho de autenticação usado pela API de transcripts."""
    api_key = os.getenv("TRANSCRIPT_API_KEY", "").strip()
    return {"X-Transcript-Key": api_key} if api_key else {}
...
async def upload_transcript_to_api(
    ...
    """
    if not _enabled("TRANSCRIPT_ENABLED", True):
        return None
    if api_url is None:
    ...
    try:
        async with aiohttp.ClientSession() as session:
            ...
            async with session.post(
                ...
                data=data,
                headers=get_transcript_api_headers(),
